use clap::Parser;
use schemagen_core::config::Config;
use schemagen_core::pipeline::{self, Registry, RunReport};
use schemagen_postgres::PostgresSchemaSource;
use schemagen_typescript::TypescriptEmitter;
use std::path::PathBuf;

#[derive(Parser)]
#[command(name = "schemagen")]
struct Cli {
    /// Path to the TOML config file
    #[arg(short, long, default_value = "schemagen.toml")]
    config: PathBuf,
}

fn main() -> std::process::ExitCode {
    match run() {
        Ok(report) => {
            for warning in &report.warnings {
                eprintln!("warning: {warning}");
            }
            println!("generated {} file(s)", report.written.len());
            std::process::ExitCode::SUCCESS
        }
        Err(error) => {
            eprintln!("error: {error}");
            std::process::ExitCode::FAILURE
        }
    }
}

fn run() -> Result<RunReport, Box<dyn std::error::Error>> {
    let cli = Cli::parse();

    let mut config = Config::from_path(&cli.config)?;
    config.resolve_env()?;

    let source =
        PostgresSchemaSource::connect(&config.database.url, config.database.schemas.clone())?;

    let mut registry = Registry::new();
    registry.register(Box::new(TypescriptEmitter));

    Ok(pipeline::run(&config, &source, &registry)?)
}
